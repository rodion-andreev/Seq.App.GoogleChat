...
namespace Seq.App.GoogleChat.Formatting
{
    static class EventFormatting
    {
        ...
        public static string SafeGetProperty(Event<LogEventData> evt, string propertyPath, bool raw = false)
        {
            ...
            while(root != null)
            {
                ...
                if (path.Count == 0)
                {
                    ...
                    return raw ? next.ToString() : GoogleChatSyntax.Escape(next.ToString());
                }
        ...
        {
            var rawValue = value?.ToString() ?? GoogleChatSyntax.Code("null");
            ...
            try
            {
                ...
                return GoogleChatSyntax.Escape(string.Format(format, rawValue));
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Could not format GoogleChat message: {Value} {Format}", value, format);
            }
